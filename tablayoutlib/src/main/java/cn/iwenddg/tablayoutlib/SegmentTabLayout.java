package cn.iwenddg.tablayoutlib;
...
import cn.iwenddg.tablayoutlib.listener.OnTabSelectListener;
import cn.iwenddg.tablayoutlib.ui.MsgView;
import cn.iwenddg.tablayoutlib.utils.FragmentChangeManager;
import cn.iwenddg.tablayoutlib.utils.UnreadMsgUtils;
...
public class SegmentTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    public SegmentTabLayout(@NonNull Context context) {
        this(context, null, 0);
    }

    public SegmentTabLayout(@NonNull Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SegmentTabLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        // 重写onDraw方法,需要调用这个方法来清除flag
        setWillNotDraw(false);
        // 用来定义他的子控件是否要在他应有的边界内进行绘制。 默认情况下，clipChild被设置为true。 也就是不允许进行扩展绘制。
